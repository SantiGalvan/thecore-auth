...
> Version: 0.0.210 | License: MIT | Author: Santiago Galvan
...
| `hasSessionKey` | `boolean` | Generate a unique session key per tab |
| `appKey` | `string` | App prefix for session key |
| `isDevelopment` | `boolean` | Development mode flag |
| `defaultTitle` | `string` | Default browser tab title |
| `routes` | `array` | Array of `{ path, title }` for page title updates |
...
The modal component. Typically controlled via `useModal`, but can also be used directly.
...
Track tab visibility, window focus, and BFCache state.
